...
class BiLSTMEngine:
    def __init__(self, config=None):
        ...
        self.pipeline = Pipeline([
            ...
            ('model', MLPRegressor(
                ...
                random_state=self.config.get('seed', 52)
            ))
        ])
        self.metrics = {}
    ...
    def train(self, data, epochs=5, sequence_length=20, horizon=1, lr=0.001, target_column='Close', y=None, **kwargs):
        if isinstance(data, pd.DataFrame):
            ...
            return {'status': 'failed', 'logs': ['Unsupported training data type for BiLSTM.'], 'metrics': {}}

        if X.size == 0 or len(y) < 10:
            return {'status': 'failed', 'logs': ['Not enough data to train BiLSTM engine.'], 'metrics': {}}
        ...
        return {'status': 'ok', 'logs': [f'BiLSTM training complete: mse={mse:.4f}'], 'metrics': {'mse': mse}}
