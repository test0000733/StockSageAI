...
class CNNLSTMEngine:
    def __init__(self, config=None):
        ...
        self.pipeline = Pipeline([
            ...
            ('model', MLPRegressor(
                ...
                random_state=self.config.get('seed', 62)
            ))
        ])
        self.metrics = {}
    ...
    def train(self, data, epochs=5, sequence_length=20, horizon=1, lr=0.001, target_column='Close', y=None, **kwargs):
        if isinstance(data, pd.DataFrame):
            ...
            return {'status': 'failed', 'logs': ['Unsupported training data type for CNN-LSTM.'], 'metrics': {}}

        if X.size == 0 or len(y) < 10:
            return {'status': 'failed', 'logs': ['Not enough data to train CNN-LSTM engine.'], 'metrics': {}}
        ...
        return {'status': 'ok', 'logs': [f'CNN-LSTM training complete: mse={mse:.4f}'], 'metrics': {'mse': mse}}
